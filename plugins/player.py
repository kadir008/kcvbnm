    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_sticker("CAACAgQAAxkBAAEDRf9hj60afr0Vto_LHdZcl5lzBJi8zgAC-gADWUH-KxD1v_4SXpzRIgQ")
        ...
        if message.command[0] == "fplay":
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply("Sen kimsin?")
                await delete_messages([message, k])
                return
        msg = await message.reply_text("Sabırlı ol...")
        if message.reply_to_message and message.reply_to_message.video:
            await msg.edit("Kontrol ediyorum...")
            type='video'
            ...
            if message.reply_to_message and message.reply_to_message.text:
                ...
                await msg.edit("Bana oynatacak bir şey vermedin. Bir videoya, bir youtube bağlantısına veya doğrudan bir müziğe yanıt ver.")
            ...
            if match:
                ...
                if has_audio_:
                    try:
                        ...
                        dur=0
                    if dur == 0:
                        await msg.edit("Bu bir canlı yayın, /stream komutunu kullanın.")
                        await delete_messages([message, msg])
                        return 
                    type="direct"
                    url=query
        ...
        if type in ["video", "audio"]:
            ...
            await msg.edit("Şarkı çalma listesine eklendi")
        elif type in ["youtube", "query", "ytdl_s"]:
            if type=="youtube":
                await msg.edit("Youtubedan video alınıyor.")
                url=yturl
            elif type=="query":
                try:
                    await msg.edit("Youtubedan video alınıyor.")
            ...
            if message.command[0] == "fplay":
                ...
                Config.playlist.append(data)
            await add_to_db_playlist(data)
            await msg.edit(f"[{title}]({url}) çalma listesine eklendi.", disable_web_page_preview=True)
        elif type == "direct":
            ...
            if message.command[0] == "fplay":
                ...
                Config.playlist.append(data)
            await add_to_db_playlist(data)        
            await msg.edit("Link çalma listesine eklendi.")
        if not Config.CALL_STATUS \
            and len(Config.playlist) >= 1:
            await msg.edit("İndiriyorum biraz bekle...")
...
async def leave_voice_chat(_, m: Message):
    if not Config.CALL_STATUS:        
        k=await m.reply("Zaten sesli sohbette değilim.")
        await delete_messages([m, k])
        return
    await leave_call()
    k=await m.reply("Sesli sohbetten çıktım. Beni özleyin :')")
    await delete_messages([m, k])
...
async def clear_play_list(client, m: Message):
    if not Config.playlist:
        k = await m.reply("Çalma listesi boş.")  
        await delete_messages([m, k])
        return
...
async def stream(client, m: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        msg=await m.reply("Alınan canlı kontrol ediliyor.")
        ...
        if match:
            ...
            if not stream_link:
                k = await msg.edit("Geçersiz link.")
                await delete_messages([m, k])
                return
        else:
            stream_link=link
        ...
        Config.msg['player']=await msg.edit(f"Canlı yayın başladı. ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ", disable_web_page_preview=True, reply_markup=await get_buttons())
        await delete_messages([m])
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgQAAxkBAAEDRf9hj60afr0Vto_LHdZcl5lzBJi8zgAC-gADWUH-KxD1v_4SXpzRIgQ", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Admin değilsin.xd', url='https://t.me/kediben')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        await m.reply("<b>Gizli</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
