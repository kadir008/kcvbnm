...
async def schedule_vc(bot, message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        yturl=""
        ysearch=""
        msg = await message.reply_text("Akış kontrol ediliyor.Biraz bekleyin.")
        if message.reply_to_message and message.reply_to_message.video:
            await msg.edit("Telegram medyası kontrol ediliyor.Biraz bekleyin.")
            type='video'
        ...
        if type in ["video", "audio"]:
            ...
            if type=="youtube":
                await msg.edit("YouTube'dan Video Alınıyor")
                url=yturl
            elif type=="query":
                try:
                    await msg.edit("Biraz bekleyin.")
                    ytquery=ysearch
            ...
            try:
                ...
                await msg.edit(
                    f"YouTube video indirme başarısız ❌\nHata:- {e}"
                    )
                LOGGER.error(str(e))
                await delete_messages([message, msg])
                return
