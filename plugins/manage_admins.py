...
@Client.on_message(filters.command(['promote', f"promote@{Config.BOT_USERNAME}"]) & sudo_filter)
async def add_admin(client, message):
    if message.reply_to_message:
        ...
        k=await message.reply("No user specified, reply to a user with /promote or pass a users user id or username.")
        await delete_messages([message, k])
        return
    ...
    k=await message.reply(f"Succesfully promoted {user.mention} as Müzikadam admin")
    await sync_to_db()
    await delete_messages([message, k])


@Client.on_message(filters.command(['demote', f"demote@{Config.BOT_USERNAME}"]) & sudo_filter)
async def remove_admin(client, message):
    if message.reply_to_message:
        ...
        k = await message.reply("No user specified, reply to a user with /demote or pass a users user id or username.")
        await delete_messages([message, k])
        return
